...
public class Player {
    ...
    public static Player createPlayer(String detail) {
        ...
        return new Player(name, LocalDate.parse(dateOfBirth), skill, numberOfMatches, runs, wickets, nationality,
                powerRating);
    }
    ...
    static Map<String, Integer> calculateNationalityCount(List<Player> list) {
        ...
        for (String country : Countries) {
            int num = (int) list.stream().filter(player -> player.getNationality().equals(country)).count();
            map.put(country, num);
        }
